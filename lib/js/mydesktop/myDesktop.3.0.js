	...
	init:function(options){
		...
		if(!$newWin.size()){ 
			...
			var $topWin=$("div.topWin"),
                ...
                wTop=myDesktop.isTypeOf(options['windowPositionTop'],"Number")?options['windowPositionTop']+dxy/2:dxy/2;
			...
			//最大化窗口
			winMaximize_btn.click(function(){ _self.winMaximize($newWin); });
			
			//最小化窗口
			winMinimize_btn.click(function(){ _self.winMinize($newWin); });
			
			//还原窗口
			winHyimize_btn.click(function(){ _self.winHyimize($newWin); });
...
myDesktop.desktop={
	...
	addApp:function(appData){
		...
		//图标添加到的桌面id并保存
		 _self.addIconSuccess(p.attr("id"),addApp);
...
myDesktop.startBtn={
	init:function(data){
		...
		for(j=0;j<arr.length;j++){
			...
			itemHtml+="<li id="+arr[j]["id"]+"><span><img src='"+arr[j]["icon"]+"'/>"+arr[j]["text"]+"</span></li>";
			}else{
			...
			$("#"+arr[j]["id"]).live("click",arr[j]["func"]);
			}	
